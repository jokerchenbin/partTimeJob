...
public class AsyncImageLoaderLocal {
	...
	public static final String APP_DIR_NAME = ".muyinghui";
	private static ExecutorService executorService;
	private final Handler handler = new Handler();
	private static AsyncImageLoaderLocal instance;

	public static AsyncImageLoaderLocal getInstance() {
		if (instance == null) {
			instance = new AsyncImageLoaderLocal();
		}
	 ...
	 * @return
	 */
	public Drawable loadSoftDrawable(
			final Map<String, SoftReference<Drawable>> imageCache, int maxSize,
	...
	public Drawable loadDrawable(final String imageUrl,
		...
		imageUrlLoadTask.add(imageUrl);
		/*
		 * fix the null point error for null imageCache and Service update by
		 * xiaohui_tu 2014-1-14 13:57
		 */
		if (executorService != null) {
			executorService.submit(new Runnable() {
				public void run() {
	 ...
	 * 
	 */
	public Drawable loadDrawable2(final String imageUrl,final String key, 
			final ImageCallback callback) {
		if (imageCache.containsKey(imageUrl+key)) {
			SoftReference<Drawable> softReference = imageCache.get(imageUrl+key);
			if (softReference.get() != null) {
				return softReference.get();
			}
		}
...
//			return null;
			...
			executorService.submit(new Runnable() {
				public void run() {
					try {
						...
						handler.post(new Runnable() {
							public void run() {
								callback.imageLoaded(drawable, imageUrl,key);
								imageUrlLoadTask.remove(imageUrl);
							}
						});
					} catch (Exception e) {
	 ...
	 * @return Drawable
	 */
	public Drawable loadDrawable(final String imageUrl,final String key,
			final ImageCallback callback) {
		if (imageCache.containsKey(imageUrl)) {
			SoftReference<Drawable> softReference = imageCache.get(imageUrl);
			if (softReference.get() != null) {
//				callback.imageLoaded(softReference.get(), imageUrl, position);
				return softReference.get();
			}
		}
//		if (containsUrl(imageUrl))
//			return null;
	...
	public Drawable loadDrawableAppointType(final String imageUrl,
		...
		executorService.submit(new Runnable() {
			public void run() {
				try {
					final Drawable drawable = fetchDrawableAppointType(
							imageUrl, imgType);
					if (drawable != null) {
						imageCache.put(imageUrl, new SoftReference<Drawable>(
								drawable));
	...
	public Drawable fetchDrawableAppointType(String imageUrl, String imgType) {
		...
		try {
			...
			if (!PHOTO_DIR.exists())
				PHOTO_DIR.mkdirs();
			localFile = new File(PHOTO_DIR, HttpMethod.getMD5Str(imageUrl)
					+ ".png");
			...
			logger.e(this.getClass().getSimpleName(), "fetchDrawable failed", e);
		} finally {
			try {
				if (fetchTaskMap.containsKey(imageUrl)) {
					fetchTaskMap.remove(imageUrl);
	...
	public Drawable fetchLocal(String urlString) {
		try {
			...
			logger.e(this.getClass().getSimpleName(), "fetchLocal failed", e);
			return null;
		}
	}

	public interface ImageCallback {
		public void imageLoaded(Drawable imageDrawable, String imageUrl, String position);
	}
